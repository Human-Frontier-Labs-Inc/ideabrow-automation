...
def initialize_repo_with_tracker(repo_url: str, tracker_path: Path, repo_name: str, requirements_path: Optional[Path] = None) -> None:
    """Initialize the repository with the progress tracker and documentation"""
...
def main():
    ...
    parser.add_argument('--requirements-path', type=str,
                       help='Path to requirements directory to copy to docs/')
    ...
    # Use project name directly (no suffix)
    ...
    try:
        ...
        # Get requirements path if provided
        requirements_path = Path(args.requirements_path) if args.requirements_path else None
        
        # Initialize with tracker and docs
        initialize_repo_with_tracker(clone_url.replace('https://', f'https://{token}@'), 
                                    tracker_path, repo_name, requirements_path)
